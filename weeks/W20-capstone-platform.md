...
# W20: Grand Capstone: Distributed Training & Serving Platform

> **Arc:** Optional capstone · **Language:** Go, Python, C++, reuses W11, W12, W15, W16, W18, W19
> **Status:** Optional / stretch week. Not required to finish the core curriculum (W00–W19).

**Prerequisite:** W11 (feature pipeline), W12 (distributed training), W15 (attention + KV cache), W16 (Chandy-Lamport snapshots), W18 (Kubernetes Operators), and W19 (Observability) all completed.
...
**Why this week exists:** W17 combines two arcs. This one chains six weeks into a single working system, end to end, on your own cluster: data in, model trained, failure survived, model served, everything observed. It's the closest thing in this curriculum to what you'd actually build on the job.
...
Project: `code/capstone-platform/` (Go for the operator and serving layer, Python for training, reusing your W11/W12/W15 code directly)
...
**Part 2: Distributed training (combine W12 and W15)**

- [ ] `train_worker.py`: combine W12's `ring_allreduce.py` with a small attention model from W15's `attention.py` (instead of the MLP from W12). Each worker trains on a shard of the feature data and exchanges gradients via ring-allreduce.
...
**Part 3: Fault tolerance (extend W16)**

- [ ] `checkpoint_coordinator.py`: adapt the Chandy-Lamport idea from W16: when a checkpoint is triggered, the coordinator waits until all workers have paused at the *same* training step (not mid-allreduce) before recording state. This is what makes the recorded checkpoint a genuinely consistent cut across workers, rather than N independent snapshots that disagree with each other.
...
**Part 4: Orchestration (extend W18)**
...
**Part 5: Serving + observability (extend W15 and W19)**

- [ ] `serve.py`: a small HTTP service (Flask/FastAPI is fine here, this isn't the distributed part) that loads the trained model and serves `/generate` using the KV cache from W15
- [ ] Instrument the serving endpoint the same way you instrumented the DD engine in W19 (Python's `prometheus_client` this time, not the C++ `prometheus-cpp` library): `requests_total` (Counter), `generation_latency_seconds` (Histogram), `active_kv_cache_entries` (Gauge)
...
- [ ] `posts/W20-capstone-platform.md`: a technical post (500–1000 words): walk a fellow engineer through the data → train → fail → recover → serve → observe loop
