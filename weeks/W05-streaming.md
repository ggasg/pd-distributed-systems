...
**Constraints:** state lives only inside `TumblingWindowAggregator`: no global mutable state, no free-floating `static` variables. Prefer `<algorithm>` calls (`std::transform`, `std::copy_if`, range-based `for`) over manual index loops where they're at least as readable; the aggregator itself will need real mutation of `sums_`, and that's fine; the discipline here is *encapsulated* mutation (private state, `const`-qualified accessors), not zero mutation. C++ won't stop anything outside the class from reaching into `sums_` if you make it public, or stop you from mutating through a `const&` via `const_cast`. Treat encapsulation here as a design discipline you're responsible for enforcing yourself, not a compiler guarantee. Use `int64_t` timestamps (milliseconds), and mark any method that doesn't mutate `sums_` as `const`.
