...
**Why measure rather than build.** Building a `Mapper`/`Reducer` abstraction with a hand-rolled shuffle leaves you owning a MapReduce framework, which is a thing you will never write again and never operate: Hadoop is legacy and Spark superseded it. What you need is the cost model, precise enough to predict which jobs will be slow before you run them, plus the ability to look at a stage DAG and see where the time goes. Spark gives you both, and the second one for free, because it draws you the picture.
