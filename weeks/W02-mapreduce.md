...
- [ ] `pip install "pyspark==4.1.0"`. Spark runs on the JVM regardless of the Python surface, so the Java 21 from [SETUP.md](../SETUP.md) satisfies it; there is no second JVM to manage.
...
- [ ] Generate a random directed graph, 100,000 nodes, average out-degree 5, written once to Parquet as `(src, dst)` pairs. Fixed seed, so both runs below see identical input.

### Step 3: `pagerank.py`

- [ ] Ten iterations of PageRank using the DataFrame API. Each iteration joins ranks to edges, divides each rank by its out-degree, aggregates contributions per destination, and applies damping at 0.85.
- [ ] Keep the edge list and the ranks as separate DataFrames. The edge list never changes across iterations and the ranks do, which is the whole shape this unit measures.

### Step 4: run it twice

- [ ] Run once with no caching at all. Record wall-clock time.
- [ ] Run again with the edge DataFrame cached: `edges.cache()` before the loop, then force materialization with a `count()` so the caching happens before the first iteration rather than during it. Record wall-clock time.
