...
> **Arc:** Optional capstone · **Language:** Python (+ Helm/YAML), reuses W09, W10, W13, W14, W16; deploys via Kubeflow Trainer from W15
...
> **Status:** Optional / stretch week. Not required to finish the core curriculum (W00 to W16).

**Prerequisite:** W09 (feature pipeline), W10 (distributed training), W13 (attention + KV cache), W14 (Chandy-Lamport snapshots), W15 (Kubernetes Operators), and W16 (Observability) all completed.

## What you'll build

A small end-to-end distributed ML platform on your kind cluster: a versioned feature pipeline feeds a tiny attention-based model that trains across multiple worker Pods via ring-allreduce, checkpoints itself so a killed worker resumes instead of retraining from scratch, runs as a `TrainJob` under the Kubeflow Trainer operator you installed in W15 (you're operating a real operator here, not extending one you wrote), lands in a model registry that records which data version produced it, and is served afterward from a specific registered version through a KV-cached inference endpoint, all instrumented with the Prometheus/Grafana stack from W16.
...
- W13: KV cache, and why it turns O(N²) generation into O(N)
- W14: Chandy-Lamport, and what a *consistent* recorded state actually means
...
Project: `code/capstone-platform/` (Python for training, coordination, and serving; Helm/YAML for deploying as a `TrainJob` via the Kubeflow Trainer operator from W15; reusing your W09/W10/W13 code directly)
...
**Part 2: Distributed training (combine W10 and W13)**

- [ ] `train_worker.py`: combine W10's `ring_allreduce.py` with a small attention model from W13's `attention.py` (instead of the MLP from W10). Each worker trains on a shard of the feature data and exchanges gradients via ring-allreduce.
...
**Part 3: Fault tolerance (extend W14)**

- [ ] `checkpoint_coordinator.py`: adapt the Chandy-Lamport idea from W14: when a checkpoint is triggered, the coordinator waits until all workers have paused at the *same* training step (not mid-allreduce) before recording state. This is what makes the recorded checkpoint a genuinely consistent cut across workers, rather than N independent snapshots that disagree with each other.
...
This is W09's lesson again, one level up. Replacing a `latest.txt` pointer with a commit log is what an open table format did for your feature data. A model registry does the same thing for models.
...
- [ ] Tag the registered version with the W09 feature-store version it trained on. That single tag is what makes the whole thing worth doing: it turns "which model is this?" into a question with an answer that reaches all the way back to the data.
...
**Part 6: Serving + observability (extend W13 and W16)**

- [ ] `serve.py`: a small HTTP service (Flask/FastAPI is fine here, this isn't the distributed part) that loads the registered model version from Part 5 and serves `/generate` using the KV cache from W13
- [ ] Instrument the serving endpoint the same way you instrumented the DD engine in W16 (Python's `prometheus_client` this time, not the Java Prometheus client library): `requests_total` (Counter), `generation_latency_seconds` (Histogram), `active_kv_cache_entries` (Gauge)
...
**What you know now, having built this, that you didn't know after W15:**
