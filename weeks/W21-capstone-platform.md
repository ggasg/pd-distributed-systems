...
This is W11's lesson again, one level up. Replacing a `latest.txt` pointer with a commit log is what an open table format did for your feature data. A model registry does the same thing for models.
...
- [ ] Tag the registered version with the W11 feature-store version it trained on. That single tag is what makes the whole thing worth doing: it turns "which model is this?" into a question with an answer that reaches all the way back to the data.
...
**Part 6: Serving + observability (extend W16 and W20)**

- [ ] `serve.py`: a small HTTP service (Flask/FastAPI is fine here, this isn't the distributed part) that loads the registered model version from Part 5 and serves `/generate` using the KV cache from W16
