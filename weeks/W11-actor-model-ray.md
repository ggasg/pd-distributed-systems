...
Project: `code/actor-training/` (Python 3.12+, `ray`, `torch`)
...
- [ ] A small PyTorch CNN for MNIST: two conv layers plus one linear layer is enough. Plain `torch.nn.Module`, nothing distributed.

### Step 2: `worker_actor.py`

- [ ] `@ray.remote class TrainerWorker`, holding a local copy of the model and optimizer as actor state, set once in `__init__` and mutated across calls. That persistence is the part a stateless task could not do.
- [ ] Methods: `compute_gradients(batch) -> grads`, `apply_gradients(grads) -> None`, `set_weights(weights) -> None`, `get_weights() -> weights`.

### Step 3: `parameter_server_actor.py`

- [ ] `@ray.remote class ParameterServer`, holding the canonical model weights.
- [ ] `push_gradients(worker_id, grads) -> None`: accumulate per worker; once all N workers for this round have pushed, average them and update the server's weights.
- [ ] `pull_weights() -> weights`.
- [ ] Ray processes calls to a single actor one at a time, so `push_gradients` can mutate the shared accumulator with no explicit lock.

### Step 4: `train.py`

- [ ] `ray.init()`, then spin up N `TrainerWorker` actors and one `ParameterServer` actor.
- [ ] Each round: workers `pull_weights()`, run a local batch through `compute_gradients()`, then `push_gradients()` to the server. Once the server has averaged, workers `pull_weights()` again for the next round.
- [ ] Five epochs on MNIST, same shard split as W09.

### Step 5: `compare.py`

- [ ] Run the same training job three ways and print wall-clock time and final accuracy for each: a single-process sequential baseline, W09's raw-socket ring-allreduce imported from `code/distributed-training/`, and this unit's Ray actor-based parameter server.
