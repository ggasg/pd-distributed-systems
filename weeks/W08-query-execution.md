...
- [ ] [DuckDB execution engine source](https://github.com/duckdb/duckdb/tree/main/src/execution): optional but worth it — a real, actively maintained vectorized query engine in C++, and one you already depend on via W09's feature store. Skim `PhysicalFilter` and how DuckDB batches rows into `DataChunk`s; that's the production version of what you're building this week.
