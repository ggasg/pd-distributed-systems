...
## What you'll build
A Ray-based actor system in Python that retrains the same data-parallel MNIST job from W10, this time using stateful actors (a pool of `TrainerWorker` actors plus a `ParameterServer` actor) instead of raw sockets, backed by a real PyTorch model instead of a hand-rolled NumPy MLP. You'll benchmark it against W10's implementation to see what the actor abstraction buys you, and what it costs.

**Scenario:** the actor model just fixed the exact hang you caused on purpose in W10, concurrent writers to shared state can't race anymore. It did nothing, though, about a worker that simply never shows up to a round, and that's a different failure with a different fix, which is what the exercise below is actually about.
...
Scenario: reimplement W10's data-parallel training job, but with two changes: workers are Ray actors instead of raw processes talking over sockets, and the model is a real PyTorch `nn.Module` instead of a hand-rolled NumPy MLP.
...
- [ ] `train.py`: `ray.init()`, spin up N `TrainerWorker` actors and 1 `ParameterServer` actor. Each round: workers `pull_weights()`, run a local batch through `compute_gradients()`, `push_gradients()` to the server; once the server has averaged, workers `pull_weights()` again for the next round. Train for 5 epochs on MNIST, same shard split as W10.
- [ ] `compare.py`: run the same training job three ways and print wall-clock time plus final accuracy for each: (1) single-process sequential baseline, (2) W10's raw-socket ring-allreduce (import directly from `code/distributed-training/`), (3) this week's Ray actor-based parameter server.
...
**Why does the actor abstraction make `push_gradients` safe without explicit locks, when W10's raw-socket workers needed you to reason about message ordering yourself?**

**Where does a single `ParameterServer` actor start to become a bottleneck at scale, and how does that connect back to why W10's ring-allreduce avoids a central coordinator entirely?**
