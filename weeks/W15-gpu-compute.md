...
**Minimum bar:** naive versus cache-blocked GEMM, both running, with measured GFLOPS for each and a defensible statement about where each one sits relative to the roofline. The CUDA path, the Go bench runner, and the auto-vectorization comparison are all extensions.
