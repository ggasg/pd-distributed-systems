...
- [ ] **DDIA Chapter 6** (2nd ed.), Replication. The largest genuine gap this curriculum had: nothing anywhere covered how systems keep more than one copy of data, which is the primary way real systems tolerate the faults this unit is named after. Read the sections on replication lag and on failover. The connection to your own work is direct rather than thematic: failover requires deciding that a leader is dead, which is the exact crashed-or-slow ambiguity you built a detector for in W03, except that here getting it wrong costs writes rather than a false alarm. Chandy-Lamport records a consistent state; replication is what there is more than one of.
