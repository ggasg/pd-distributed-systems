...
**Rank your four implementations (C++ `IncrementalAggregateView`, ClickHouse MV, Spark stateful aggregation, and — if you did the stretch — pg_ivm) from closest to furthest from true DD semantics (tracks retractions, recomputes only the affected delta). Where does each one cut a corner, and why might that corner-cutting be the right engineering trade-off for that system's actual use case?**
