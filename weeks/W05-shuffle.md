...
**Part 1, the mechanism, which you build.** A working shuffle in Java: map tasks that split their output into per-partition spill files, reduce tasks that fetch their own partition from every map task, and a partitioner that decides which key goes where. The write-then-fetch structure is invisible from outside a system that does it for you, which is why this half is built rather than measured.
...
**The terms the rest of the unit assumes:**

- **Partition key**: the field whose value decides which partition a row goes to, usually via `hash(key) % numPartitions`.
- **Skew**: an uneven distribution of rows across those partitions. Real keys are almost never uniform, so some partitions end up much larger than others.
- **Hot key**: the extreme case, a single key value holding enough rows that its partition dwarfs the rest. In practice this is a null placeholder, a default account ID, or your largest customer.
- **Straggler**: the one task still running long after its peers finished. A stage cannot complete until its slowest task does, so wall-clock time for the whole stage collapses to that single task's duration.

Skew is the cause, the straggler is the symptom you see, and every fix in Part 2 works by changing which rows land where.
...
### Step 1: `Partitioner.java`

- [ ] `sealed interface Partitioner permits HashPartitioner, RangePartitioner { int partitionFor(String key); }`, the same sealed-interface idiom as W04's `StreamItem`. `HashPartitioner` is `Math.floorMod(key.hashCode(), numPartitions)`; use `floorMod`, not `%`, because `hashCode()` can be negative and `%` in Java preserves the sign, which would hand you a negative array index. `RangePartitioner` takes a sorted array of split-point keys and binary-searches (`Arrays.binarySearch`) to find the partition.

### Step 2: `MapTask.java`

- [ ] Takes a `List<Record>` and a `Partitioner`, and writes one file per reduce partition into `spill/map-<mapId>/part-<partitionId>`. Each file is plain text, one `key,value` pair per line.
- [ ] Write R files rather than one, so a reduce task can read only what belongs to it.

### Step 3: `ReduceTask.java`

- [ ] For partition `p`, read `spill/map-*/part-<p>` from every map task, sum values per key, and write `output/part-<p>`. Return the number of records processed; you need it later.
- [ ] Write the aggregation as `Collectors.groupingBy(Record::key, Collectors.summingInt(Record::value))`. That single line is the entire reduce side of a shuffle. Everything else in Part 1 exists because you cannot call it when the data does not fit in one JVM: the rows for a key have to be brought together first.

### Step 4: `Shuffle.java`

- [ ] Wire M map tasks and R reduce tasks together and run them. Single JVM, plain threads, or sequentially, which is easier to debug.
- [ ] The filesystem stands in for the network here. Real spill-to-disk then fetch is what Spark does, so this is a simplification of scale rather than of structure.
...
Part 1 taught the mechanism. This half is about finding the problem in a system you did not write, using the evidence where it will actually be.
...
- [ ] `datagen/gen.py`: emit `orders` and `customers` to Parquet, writing under `data/`, which is gitignored so generated tables stay out of the repo. It takes `--scale`, multiplying base row counts of 10,000,000 orders over 200,000 customers, and `--skew`, a Zipf exponent applied to `customer_id`. Fixed `--seed` so both runs below are reproducible.
...
Worth noticing while you are in there: those percentages barely move if you change the customer count. Zipf's head share is governed by the exponent, not by how many distinct keys exist, which is why "we have millions of customers" is never on its own an argument that a job will not skew.

- [ ] `skew_job.py`: run the same `groupBy("customer_id")` and aggregation over both datasets. A handful of keys taking most of the rows is what real data looks like: a few huge customers, one null-ish default value, one bot account.
