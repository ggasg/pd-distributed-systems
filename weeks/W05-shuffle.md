...
**Part 1, the mechanism, which you build.** A working shuffle in Java: map tasks that split their output into per-partition spill files, reduce tasks that fetch their own partition from every map task, and a partitioner that decides which key goes where. You build this one rather than measure it, because the shuffle is the mechanism every other unit in the arc refers back to, and there is no way to see the write-then-fetch structure from outside a system that does it for you.
...
**Four more terms, since the rest of the unit assumes them.** The **partition key** is the field whose value decides which partition a row goes to, usually via `hash(key) % numPartitions`. **Skew** is an uneven distribution of rows across those partitions: real keys are almost never uniform, so some partitions end up much larger than others. A **hot key** is the extreme case, a single key value holding enough rows that its partition dwarfs the rest, which is what a null placeholder, a default account ID, or your largest customer looks like in practice. A **straggler** is the consequence: the one task still running long after its peers finished, which matters because a stage cannot complete until its slowest task does, so wall-clock time for the whole stage collapses to that single task's duration. Skew is the cause, the straggler is the symptom you actually see, and every fix in Part 2 works by changing which rows land where.
