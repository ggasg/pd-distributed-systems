---
week_number: 99
status: not-started
---

# WXX: [Topic]

> **Arc:** [Arc Name] · **Language:** [Primary Language]

## What you'll build
[One paragraph. Start with: "A [thing] in [language] that [does what]." Be specific enough that someone can start without reading the rest. Name the files they'll create.]

---

## Read
- [ ] [Paper or book chapter title](URL): Author(s), Year, [what to focus on, estimated time]
- [ ] [Second source](URL): [what to focus on]

**Key question:** [One question that forces synthesis of the reading. Should not be answerable by skimming. Something like "Why does X have to be Y, and what breaks if it isn't?"]

---

## Code

Project: `code/[directory]/` ([Language + version])

- [ ] `[FileName.ext]`: [what it does, named fields or methods to implement, any constraints]
- [ ] `[FileName.ext]`: [what it does]
- [ ] `[FileName.ext]`: [what it does]
- [ ] Verify: [exact command to run that proves correctness: a test, a benchmark output, a curl, etc.]

**Minimum bar:** [What "done" looks like. Be specific: "X test passes", "benchmark shows Nx speedup", "3 nodes converge", etc.]

---

## Rehearse it in Python first (optional, 20 minutes)

> Include this only if it earns its place: the unit is built in a language you're still learning, or the exercise teaches something the build itself doesn't. Skip it otherwise.

**[Data structure or algorithm name]**: [one sentence on why it's relevant to this week]

```python
# [filename].py
# [implementation]
```

**Connection:** [Explicit link between the Python code above and the week's main implementation task]

---

## Reflect

**What clicked:**

**What surprised me:**

**[Week-specific question 1, about the core concept]:**

**[Week-specific question 2, connecting to a system you know]:**

**What I'd do differently:**
